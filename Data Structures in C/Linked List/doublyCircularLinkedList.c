// Implementation of Circular Doubly Linked List in C Programming Language
#include<stdio.h>
#include<stdlib.h>
...
int main(){
    printf("\t\t\t\tIMPLEMENTATION of CIRCULAR DOUBLY LINKED LIST in C \n");
    int option;
    do{
...
struct node* delete_end(struct node* head){
    ...
    if(head != NULL){
        if(head->next == head){
            ...
            prevPtr->next = head;
            head->prev = prevPtr;
            //printf("\nAfter:- Start = %d %x\tLast = %d %x", head->rollNo, head->next, prevPtr->rollNo, prevPtr->next);
...
void display_list(struct node* head){
    ...
    if(head == NULL)
        ...
        while(ptr->next != head){
            printf("\nCurrent Node:- Previous Node = %x \tData = %d & Address = %x\tNext Node = %x", ptr->prev, ptr->rollNo, ptr, ptr->next);
            ptr = ptr->next;
        }
        printf("\nCurrent Node:- Previous Node = %x\tData = %d & Address = %x\tNext Node = %x", ptr->prev, ptr->rollNo, ptr, ptr->next);
    } 
